...
**`RadarLocationSourceOffline`** — New location source enum value (`OFFLINE_DETECTION`) to identify events and users generated by the offline event manager.
...
4. **Synthetic user** (`buildSyntheticUser`): Retrieves the cached `RadarUser` from `RadarState` (persisted on every successful track) and constructs a new user with updated location, geofences, and source (`OFFLINE_DETECTION`), while preserving identity fields (`_id`, `userId`, `deviceId`) and geographic context (`country`, `state`, `dma`, `postalCode`, `trip`).
