...
| `make check-objc-linkage` | Build Release and verify public Objective-C class symbols plus a linker fixture |
...
- When a migrated implementation keeps its public Objective-C header, use `@objc @implementation extension` for the imported class. Do not replace it with a standalone `@objc(ClassName)` Swift class; run `make check-objc-linkage` before opening the PR.
