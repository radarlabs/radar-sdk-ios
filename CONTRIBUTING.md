...
| `make ci-build-example` | Build the Example app in Release, link every public Objective-C and Swift class, and compile the umbrella as non-modular Objective-C++ |
...
- When you migrate a public class, follow "Public classes implemented in Swift" in `AGENTS.md`. In short: declare it once in Swift as `@objc(ClassName) @objcMembers public final class`, keep exactly the old header's public API (covered by `RadarPublicAPICompatibilityTests`), move the header docs onto the Swift members, reduce the header to a forwarder, and declare internal Objective-C initializers in `ClassName+Internal.h`. Run `make ci-build-example` and `make lint` before opening the PR.
