...
| `make ci-build-example` | Build the Example app in Release and link every public Objective-C and Swift class |
...
- When you migrate a public class, make it `@objc(ClassName) @objcMembers public class ClassName` with the same name in Swift and Objective-C, and mark its public API `public`. Keep the names, types, nullability, and selectors of the old header, and cover them in `RadarPublicAPICompatibilityTests`. Reduce the handwritten public header to a forwarder that imports `<RadarSDK/RadarSDK.h>` (plus any enums or constants) and list it in `RadarSDK.h`. Other public headers should forward-declare the class with `@class`. Declare internal Objective-C initializers in `ClassName+Internal.h`, not in public headers. Run `make ci-build-example` and `make lint` before opening the PR.
