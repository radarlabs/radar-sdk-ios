...
- When you migrate a public class, make it `@objc(ClassName) @objcMembers public class ClassName` with the same name in Swift and Objective-C, and mark its public API `public`. Remove its `@interface` from the handwritten public header, keeping only enums or constants. Other public headers should forward-declare it with `@class`. Declare internal Objective-C initializers in `ClassName+Internal.h`, not in public headers. Run `make ci-build-example` and `make lint` before opening the PR.
