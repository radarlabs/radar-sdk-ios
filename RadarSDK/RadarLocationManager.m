...
- (instancetype)init {
    ...
    if (self) {
        ...
        if ([RadarSettings useRadarModifiedBackgroundLocation]) {
...
- (void)handleLocation:(CLLocation *)location source:(RadarLocationSource)source beacons:(NSArray<RadarBeacon *> *)beacons {
    ...
    if (options.stopDistance > 0 && options.stopDuration > 0) {
        ...
        if (location && lastMovedLocation && lastMovedAt) {
            ...
            stopped = (distance <= options.stopDistance && duration >= options.stopDuration) || (![RadarSettings useRadarModifiedBackgroundLocation] && arrival);

            [[RadarLogger sharedInstance]
                logWithLevel:RadarLogLevelDebug
                     message:[NSString stringWithFormat:@"Calculating stopped | stopped = %d; arrival = %d; distance = %f; duration = %f; location.timestamp = %@; lastMovedAt = %@", stopped, arrival, distance, duration, location.timestamp, lastMovedAt]];
