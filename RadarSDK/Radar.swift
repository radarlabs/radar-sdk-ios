    ...
    public func mockTracking(origin: CLLocation, destination: CLLocation, mode: RadarRouteMode, steps: Int, interval: TimeInterval, onTrack: @escaping ([String: Any]) -> Void) async {
        do {
            ...
            if mode.contains(.bike) {
                ...
                route = routes.car
            } else if mode.contains(.foot) {
                route = routes.foot
            } else if mode.contains(.motorbike) {
                route = routes.motorbike
            } else if mode.contains(.truck) {
                route = routes.truck
            } else {
                // no route selected
                return
            ...
            guard let coordinates = route?.geometry.coordinates else {
