...
struct RadarAPIHelperTests {
    ...
    @Test("Prepared headers are preserved when sending and retrying")
    func radarRequestPreservesPreparedHeaders() async throws {
        let session = RetryTestSession(failures: [.networkConnectionLost])
        let helper = RadarAPIHelper(session: session)
        let headers = [
            "Authorization": "prepared-test-key",
            "Origin": "https://example.test",
            "X-Radar-Product": "prepared-product",
            "X-Radar-SDK-Version": "prepared-version",
            "Content-Type": "application/json",
        ]

        _ = try await helper.radarRequest(
            host: .verifiedSecondaryHost,
            method: "POST",
            url: "reveal/risk",
            headers: headers,
            body: ["fraudPayload": "encrypted-envelope"],
        ...
        for request in requests {
            ...
            #expect(request.allHTTPHeaderFields?.count == headers.count)
            ...
            #expect(body == ["fraudPayload": "encrypted-envelope"])
        }
