...
#define OCMStub(invocation)                                                                                                                                                        \
    ({                                                                                                                                                                             \
        _OCMSilenceWarnings(                                                                                                                                                       \
            [OCMMacroState beginStubMacro]; OCMStubRecorder *recorder = nil; @try { invocation; } @finally { recorder = [OCMMacroState endStubMacro]; } recorder;);                \
    })

#define OCMExpect(invocation)                                                                                                                                                      \
    ({                                                                                                                                                                             \
        _OCMSilenceWarnings(                                                                                                                                                       \
            [OCMMacroState beginExpectMacro]; OCMStubRecorder *recorder = nil; @try { invocation; } @finally { recorder = [OCMMacroState endExpectMacro]; } recorder;);            \
    })

#define OCMReject(invocation)                                                                                                                                                      \
    ({                                                                                                                                                                             \
        _OCMSilenceWarnings(                                                                                                                                                       \
            [OCMMacroState beginRejectMacro]; OCMStubRecorder *recorder = nil; @try { invocation; } @finally { recorder = [OCMMacroState endRejectMacro]; } recorder;);            \
    })

#define OCMClassMethod(invocation) _OCMSilenceWarnings([[OCMMacroState globalState] switchToClassMethod]; invocation;);
...
#define _OCMVerify(invocation)                                                                                                                                                     \
    ({                                                                                                                                                                             \
        _OCMSilenceWarnings(                                                                                                                                                       \
            [OCMMacroState beginVerifyMacroAtLocation:OCMMakeLocation(self, __FILE__, __LINE__)]; @try { invocation; } @finally { [OCMMacroState endVerifyMacro]; });              \
    })
...
#define _OCMSilenceWarnings(macro)                                                                                                                                                 \
    ({_Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wunused-value\"") _Pragma("clang diagnostic ignored \"-Wunused-getter-return-value\"")                 \
          _Pragma("clang diagnostic ignored \"-Wstrict-selector-match\"") macro _Pragma("clang diagnostic pop")})
