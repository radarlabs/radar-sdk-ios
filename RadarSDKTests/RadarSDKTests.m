...
#pragma mark - RadarNotificationHelper notificationUniqueIdentifierForRequest

- (UNNotificationRequest *)beaconRequestWithMajor:(nullable NSNumber *)major minor:(nullable NSNumber *)minor {
    NSUUID *uuid = [[NSUUID alloc] initWithUUIDString:@"5A4BCFCE-174E-4BAC-A814-092E77F6B7E5"];
    CLBeaconRegion *region;
    if (major && minor) {
        region = [[CLBeaconRegion alloc] initWithProximityUUID:uuid major:major.unsignedShortValue minor:minor.unsignedShortValue identifier:@"beacon"];
    } else if (major) {
        region = [[CLBeaconRegion alloc] initWithProximityUUID:uuid major:major.unsignedShortValue identifier:@"beacon"];
    } else {
        region = [[CLBeaconRegion alloc] initWithProximityUUID:uuid identifier:@"beacon"];
    }
    ...
    return [UNNotificationRequest requestWithIdentifier:@"radar_beacon_5A4BCFCE" content:content trigger:trigger];
}

- (void)testNotificationUniqueIdentifierForRequestBeaconRegionSameMajorMinor {
    UNNotificationRequest *first = [self beaconRequestWithMajor:@1 minor:@1];
    UNNotificationRequest *second = [self beaconRequestWithMajor:@1 minor:@1];
    XCTAssertEqualObjects([RadarNotificationHelper notificationUniqueIdentifierForRequest:first],
                          [RadarNotificationHelper notificationUniqueIdentifierForRequest:second],
                          @"same identifier, content, and beacon region should hash the same");
}

- (void)testNotificationUniqueIdentifierForRequestBeaconRegionChangedMinor {
    // Same notification identifier + content (as RadarBeaconManager derives the identifier from
    // UUID alone), but the beacon's minor changed - the region must be reflected in the signature
    // or a changed beacon campaign is silently treated as unchanged and never re-armed.
    UNNotificationRequest *first = [self beaconRequestWithMajor:@1 minor:@1];
    UNNotificationRequest *second = [self beaconRequestWithMajor:@1 minor:@2];
    XCTAssertNotEqualObjects([RadarNotificationHelper notificationUniqueIdentifierForRequest:first],
                             [RadarNotificationHelper notificationUniqueIdentifierForRequest:second],
                             @"a changed minor must change the signature");
}

- (void)testNotificationUniqueIdentifierForRequestCircularRegionChangedRadius {
    CLCircularRegion *smallRegion = [[CLCircularRegion alloc] initWithCenter:CLLocationCoordinate2DMake(40.0, -74.0) radius:100 identifier:@"geofence"];
    CLCircularRegion *largeRegion = [[CLCircularRegion alloc] initWithCenter:CLLocationCoordinate2DMake(40.0, -74.0) radius:250 identifier:@"geofence"];
    UNMutableNotificationContent *content = [UNMutableNotificationContent new];
    content.title = @"Geofence";
    UNNotificationRequest *first = [UNNotificationRequest requestWithIdentifier:@"radar_geofence_1"
                                                                        content:content
                                                                        trigger:[UNLocationNotificationTrigger triggerWithRegion:smallRegion repeats:NO]];
    UNNotificationRequest *second = [UNNotificationRequest requestWithIdentifier:@"radar_geofence_1"
                                                                         content:content
                                                                         trigger:[UNLocationNotificationTrigger triggerWithRegion:largeRegion repeats:NO]];
    XCTAssertNotEqualObjects([RadarNotificationHelper notificationUniqueIdentifierForRequest:first],
                             [RadarNotificationHelper notificationUniqueIdentifierForRequest:second],
                             @"a changed radius must change the signature");
}
