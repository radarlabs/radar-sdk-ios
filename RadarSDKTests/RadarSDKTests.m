...
#pragma mark - SyncManager Test Helpers
...
- (RadarGeofence *)createTestGeofenceWithId:(NSString *)identifier
    ...
    return [[RadarGeofence alloc] initWithId:identifier description:@"Test Geofence" tag:@"test" externalId:identifier metadata:nil operatingHours:nil geometry:geometry dwellThreshold:nil geofenceStopDetection:nil];
}

- (RadarGeofence *)createTestPolygonGeofenceWithId:(NSString *)identifier
                                       coordinates:(NSArray<RadarCoordinate *> *)coordinates
                                            center:(RadarCoordinate *)center
                                            radius:(double)radius {
    RadarPolygonGeometry *geometry = [[RadarPolygonGeometry alloc] initWithCoordinates:coordinates center:center radius:radius];
    return [[RadarGeofence alloc] initWithId:identifier description:@"Test Polygon Geofence" tag:@"test" externalId:identifier metadata:nil operatingHours:nil geometry:geometry dwellThreshold:nil geofenceStopDetection:nil];
}
...
- (RadarGeofence *)createTestGeofenceWithId:(NSString *)identifier
                                  ...
                                  longitude:(double)lng
                                     radius:(double)radius
                             dwellThreshold:(NSNumber *)dwellThreshold
                      geofenceStopDetection:(NSNumber *)geofenceStopDetection {
    RadarCoordinate *center = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(lat, lng)];
    RadarCircleGeometry *geometry = [[RadarCircleGeometry alloc] initWithCenter:center radius:radius];
    return [[RadarGeofence alloc] initWithId:identifier description:@"Test Geofence" tag:@"test" externalId:identifier metadata:nil operatingHours:nil geometry:geometry dwellThreshold:dwellThreshold geofenceStopDetection:geofenceStopDetection];
}
...
- (void)clearSyncManagerState {
    [RadarState setSyncedGeofences:nil];
    [RadarState setSyncedBeacons:nil];
    [RadarState setSyncedPlaces:nil];
    [RadarState setLastSyncedGeofenceIds:nil];
    [RadarState setLastSyncedBeaconIds:nil];
    [RadarState setLastSyncedPlaceIds:nil];
    [RadarState setGeofenceEntryTimestamps:nil];
    [RadarState setDwellEventsFired:nil];
...
- (void)test_SyncManager_shouldTrack_noSyncedRegion {
    [self clearSyncManagerState];
    ...
    options.syncLocations = RadarTrackingOptionsSyncEvents;
...
- (void)test_SyncManager_shouldTrack_outsideSyncedRegion {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_shouldTrack_geofenceEntry {
    [self clearSyncManagerState];
    ...
    [RadarState setSyncedGeofences:@[geofence]];
    
    [RadarState setLastSyncedGeofenceIds:@[]];
...
- (void)test_SyncManager_shouldTrack_geofenceExit {
    [self clearSyncManagerState];
    ...
    [RadarState setLastSyncedGeofenceIds:@[@"geofence1"]];
...
- (void)test_SyncManager_shouldNotTrack_noStateChange {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_getGeofences_insideCircle {
    [self clearSyncManagerState];
    ...
    NSArray<RadarGeofence *> *geofences = [RadarSyncManager getGeofencesFor:location checkingForExit:NO];
    XCTAssertEqual(geofences.count, 1);
    XCTAssertEqualObjects(geofences.firstObject._id, @"geofence1");
}

- (void)test_SyncManager_getGeofences_outsideCircle {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_getGeofences_noNearbyGeofences {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_geofenceStateChanged_entry {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_geofenceStateChanged_exit {
    [self clearSyncManagerState];
    
    RadarGeofence *geofence = [self createTestGeofenceWithId:@"geofence1" latitude:kTestLatitudeFar longitude:kTestLongitude radius:50];
    [RadarState setSyncedGeofences:@[geofence]];
    [RadarState setLastSyncedGeofenceIds:@[@"geofence1"]];
    [RadarState setGeofenceEntryTimestamps:@{@"geofence1": [NSDate date]}];
...
- (void)test_SyncManager_geofenceStateChanged_noChange {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_getBeacons_outsideRange {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_beaconStateChanged_entry {
    [self clearSyncManagerState];
    
    RadarBeacon *beacon = [self createTestBeaconWithId:@"beacon1" latitude:kTestLatitude longitude:kTestLongitude];
    [RadarState setSyncedBeacons:@[beacon]];
    [RadarState setLastSyncedBeaconIds:@[]];  // Was not near any beacon
...
- (void)test_SyncManager_beaconStateChanged_exit {
    [self clearSyncManagerState];
    
    RadarBeacon *beacon = [self createTestBeaconWithId:@"beacon1" latitude:kTestLatitudeFar longitude:kTestLongitude];
    [RadarState setSyncedBeacons:@[beacon]];
    [RadarState setLastSyncedBeaconIds:@[@"beacon1"]];
...
- (void)test_SyncManager_getPlaces_outsideRadius {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_placeStateChanged_entry {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_placeStateChanged_exit {
    [self clearSyncManagerState];
    
    RadarPlace *place = [self createTestPlaceWithId:@"place1" latitude:kTestLatitudeFar longitude:kTestLongitude];
    [RadarState setSyncedPlaces:@[place]];
    [RadarState setLastSyncedPlaceIds:@[@"place1"]];
...
- (void)test_SyncManager_isOutsideSyncedRegion_nil {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_isOutsideSyncedRegion_inside {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_isOutsideSyncedRegion_outside {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_isPointInsideCircle_inside {
...
- (void)test_SyncManager_isPointInsideCircle_outside {
...
- (void)test_SyncManager_multipleGeofences_shouldTrackWhenCrossingNearestBoundary {
    [self clearSyncManagerState];
    ...
    [RadarState setSyncedGeofences:@[geofenceA, geofenceB]];
...
- (void)test_SyncManager_multipleGeofences_detectsCorrectGeofences {
    [self clearSyncManagerState];
    ...
    [RadarState setSyncedGeofences:@[geofenceA, geofenceB, geofenceC]];
    
    CLLocation *location = [[CLLocation alloc] initWithLatitude:kTestLatitude longitude:kTestLongitude];

    NSArray<RadarGeofence *> *detectedGeofences = [RadarSyncManager getGeofencesFor:location checkingForExit:NO];
...
- (void)test_SyncManager_getGeofences_insidePolygon {
    [self clearSyncManagerState];
    
    // Square polygon ~200m around test point
    RadarCoordinate *c1 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitude + 0.001, kTestLongitude - 0.001)];
    RadarCoordinate *c2 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitude + 0.001, kTestLongitude + 0.001)];
    RadarCoordinate *c3 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitude - 0.001, kTestLongitude + 0.001)];
    RadarCoordinate *c4 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitude - 0.001, kTestLongitude - 0.001)];
    RadarCoordinate *center = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitude, kTestLongitude)];

    RadarGeofence *geofence = [self createTestPolygonGeofenceWithId:@"poly1" coordinates:@[c1, c2, c3, c4, c1] center:center radius:150];
    [RadarState setSyncedGeofences:@[geofence]];

    CLLocation *location = [[CLLocation alloc] initWithLatitude:kTestLatitude longitude:kTestLongitude];
    NSArray<RadarGeofence *> *geofences = [RadarSyncManager getGeofencesFor:location checkingForExit:NO];
    XCTAssertEqual(geofences.count, 1);
    XCTAssertEqualObjects(geofences.firstObject._id, @"poly1");
}

- (void)test_SyncManager_getGeofences_outsidePolygon {
    [self clearSyncManagerState];
    
    // Square polygon ~200m around a point far from the user
    RadarCoordinate *c1 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitudeFar + 0.001, kTestLongitude - 0.001)];
    RadarCoordinate *c2 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitudeFar + 0.001, kTestLongitude + 0.001)];
    RadarCoordinate *c3 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitudeFar - 0.001, kTestLongitude + 0.001)];
    RadarCoordinate *c4 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitudeFar - 0.001, kTestLongitude - 0.001)];
    RadarCoordinate *center = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitudeFar, kTestLongitude)];
...
- (void)test_SyncManager_getGeofences_mixedCircleAndPolygon {
    [self clearSyncManagerState];
    
    RadarGeofence *circleGeofence = [self createTestGeofenceWithId:@"circle1" latitude:kTestLatitude longitude:kTestLongitude radius:100];
    RadarCoordinate *c1 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitude + 0.001, kTestLongitude - 0.001)];
    RadarCoordinate *c2 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitude + 0.001, kTestLongitude + 0.001)];
    RadarCoordinate *c3 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitude - 0.001, kTestLongitude + 0.001)];
    RadarCoordinate *c4 = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitude - 0.001, kTestLongitude - 0.001)];
    RadarCoordinate *center = [[RadarCoordinate alloc] initWithCoordinate:CLLocationCoordinate2DMake(kTestLatitude, kTestLongitude)];
    RadarGeofence *polyGeofence = [self createTestPolygonGeofenceWithId:@"poly1" coordinates:@[c1, c2, c3, c4, c1] center:center radius:150];
    
    [RadarState setSyncedGeofences:@[circleGeofence, polyGeofence]];
    
    CLLocation *location = [[CLLocation alloc] initWithLatitude:kTestLatitude longitude:kTestLongitude];
    NSArray<RadarGeofence *> *geofences = [RadarSyncManager getGeofencesFor:location checkingForExit:NO];
    XCTAssertEqual(geofences.count, 2);
    
    NSSet *detectIds = [NSSet setWithArray:[geofences valueForKey:@"_id"]];
    XCTAssertTrue([detectIds containsObject:@"circle1"]);
    XCTAssertTrue([detectIds containsObject:@"poly1"]);
}

- (void)test_SyncManager_getGeofences_bufferedEntry {
    [self clearSyncManagerState];
    
    //Geofence with 100m radius at test location
    RadarGeofence *geofence = [self createTestGeofenceWithId:@"geofence1" latitude:kTestLatitude longitude:kTestLongitude radius:100];
    [RadarState setSyncedGeofences:@[geofence]];
    
    //Enable buffer entries in sdk config
    RadarSdkConfiguration *config = [[RadarSdkConfiguration alloc] initWithDict:@{@"bufferGeofenceEntries": @YES}];
    [RadarSettings setSdkConfiguration:config];
    
    //User is ~110m away (outside radius) but with 20m accuracy -> effective radius = 120m
    double offsetLat = kTestLatitude + 0.001; //~111m north
    CLLocation *location = [[CLLocation alloc] initWithCoordinate:CLLocationCoordinate2DMake(offsetLat, kTestLongitude) altitude:0 horizontalAccuracy:20 verticalAccuracy:10 timestamp:[NSDate date]];
    
    NSArray<RadarGeofence *> *geofences = [RadarSyncManager getGeofencesFor:location checkingForExit:NO];
    XCTAssertEqual(geofences.count, 1);
}

- (void)test_SyncManager_getGeofences_bufferingDisabled {
    [self clearSyncManagerState];
    
    RadarGeofence *geofence = [self createTestGeofenceWithId:@"geofence1" latitude:kTestLatitude longitude:kTestLongitude radius:100];
    [RadarState setSyncedGeofences:@[geofence]];
    
    // Disable buffer entries in sdk config
    RadarSdkConfiguration *config = [[RadarSdkConfiguration alloc] initWithDict:@{@"bufferGeofenceEntries": @NO}];
    [RadarSettings setSdkConfiguration:config];
    
    // Same location as above (~110m away, 20m accuracy) but buffering off -> not inside
    double offsetLat = kTestLatitude + 0.001;
    CLLocation *location = [[CLLocation alloc] initWithCoordinate:CLLocationCoordinate2DMake(offsetLat, kTestLongitude) altitude:0 horizontalAccuracy:20 verticalAccuracy:10 timestamp:[NSDate date]];
...
- (void)test_SyncManager_geofenceEntry_stopDetectionBlocks {
    [self clearSyncManagerState];
    [RadarState setStopped:NO];
    
    // Geofence with stop detection enabled
    RadarGeofence *geofence = [self createTestGeofenceWithId:@"geofence1" latitude:kTestLatitude longitude:kTestLongitude radius:100 dwellThreshold:nil geofenceStopDetection:@YES];
    [RadarState setSyncedGeofences:@[geofence]];
    [RadarState setLastSyncedGeofenceIds:@[]];
    
    // User is inside the geofence but NOT stopped
    CLLocation *location = [[CLLocation alloc] initWithLatitude:kTestLatitude longitude:kTestLongitude];
    
    // hasGeofenceStateChanged should return NO because stop detection blocks the entry
    BOOL changed = [RadarSyncManager hasGeofenceStateChangedWithLocation:location];
    XCTAssertFalse(changed);
}

- (void)test_SyncManager_geofenceEntry_stopDetectionAllows {
    [self clearSyncManagerState];
...
- (void)test_SyncManager_geofenceDwell_thresholdReached {
    [self clearSyncManagerState];
    ...
    // User entered 10 minutes ago
    NSDate *tenMinutesAgo = [NSDate dateWithTimeIntervalSinceNow:-600];
    [RadarState setGeofenceEntryTimestamps:@{@"geofence1": tenMinutesAgo}];
    
    // Set project-level dwell threshold to 5 minutes
    RadarSdkConfiguration *config = [[RadarSdkConfiguration alloc] initWithDict:@{@"defaultGeofenceDwellThreshold": @5}];
    [RadarSettings setSdkConfiguration:config];
...
- (void)test_SyncManager_geofenceDwell_thresholdNotReached {
    [self clearSyncManagerState];
    ...
    // User entered 1 minute ago
    NSDate *oneMinuteAgo = [NSDate dateWithTimeIntervalSinceNow:-60];
    [RadarState setGeofenceEntryTimestamps:@{@"geofence1": oneMinuteAgo}];
...
- (void)test_SyncManager_geofenceDwell_perGeofenceOverride {
    [self clearSyncManagerState];
    
    // Geofence with per-geofence dwell threshold of 2 minutes (no project-level threshold)
    RadarGeofence *geofence = [self createTestGeofenceWithId:@"geofence1" latitude:kTestLatitude longitude:kTestLongitude radius:100 dwellThreshold:@2 geofenceStopDetection:nil];
    [RadarState setSyncedGeofences:@[geofence]];
    [RadarState setLastSyncedGeofenceIds:@[@"geofence1"]];
    
    // User entered 3 minutes ago
    NSDate *threeMinutesAgo = [NSDate dateWithTimeIntervalSinceNow:-180];
    [RadarState setGeofenceEntryTimestamps:@{@"geofence1": threeMinutesAgo}];
...
- (void)test_SyncManager_geofenceDwell_alreadyFired {
    [self clearSyncManagerState];
    ...
    NSDate *tenMinutesAgo = [NSDate dateWithTimeIntervalSinceNow:-600];
    [RadarState setGeofenceEntryTimestamps:@{@"geofence1": tenMinutesAgo}];
    [RadarState setDwellEventsFired:@[@"geofence1"]];
...
- (void)test_SyncManager_isNearSyncedRegionBoundary_near {
    [self clearSyncManagerState];
    
    // Synced region with 1000m radius
    CLCircularRegion *syncedRegion = [[CLCircularRegion alloc] initWithCenter:CLLocationCoordinate2DMake(kTestLatitude, kTestLongitude) radius:1000 identifier:@"synced"];
    [RadarState setSyncedRegion:syncedRegion];
    
    // User is 900m from center -> 100m from edge -> within 20% threshold (200m)
    CLLocation *location = [[CLLocation alloc] initWithLatitude:kTestLatitude + 0.0081 longitude:kTestLongitude];

    BOOL near = [RadarSyncManager isNearSyncedRegionBoundaryWithLocation:location];
    XCTAssertTrue(near);
}

- (void)test_SyncManager_isNearSyncedRegionBoundary_notNear {
    [self clearSyncManagerState];
    ...
    // User is at center → 1000m from edge → well outside 20% threshold (200m)
    CLLocation *location = [[CLLocation alloc] initWithLatitude:kTestLatitude longitude:kTestLongitude];
