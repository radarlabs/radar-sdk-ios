...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate, CLLocationManagerDelegate, RadarDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        Radar.logConversion(name: "custom_event", metadata: ["data": "test"]) { (status, location, events, user) in
        ...
        Radar.logConversion(name: "custom_event_with_location", location: customLocation, metadata: ["data": "test"]) { (status, location, events, user) in
            if let customEvent = events?.first,
               customEvent.type == .custom {
                print("Custom type: \(customEvent.customType!)")
            }
