...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate, CLLocationManagerDelegate, RadarDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        Radar.sendEvent(type: "app_launched", metadata: ["data": "test"]) { (status, location, events, user) in
        ...
        Radar.getLocation { (status, location, stopped) in
            Radar.sendEvent(type: "app_launched_manual", location: location, metadata: ["data": "test"]) { (status, location, events, user) in
                print("Send event: status = \(Radar.stringForStatus(status)); location = \(String(describing: location)); events = \(String(describing: events)); user = \(String(describing: user))")
            }
        }
